...
class Mapper:
    ...
    def minUpdateTimeElapsed(self, key, time):
        ...
        # log.debug("%s: Curr update %s, last update %s, diff: %s", key, time, self.mapping[key]['lastupdate'], diff)
        if diff * 1000 >= self.mapping[key]["minupdatedelay"]:
            self.mapping[key]["lastupdate"] = time
            log.debug(
                "minUpdateTimeElapsed(%s) diff:%d, min:%s",
                key,
                diff * 1000,
                self.mapping[key]["minupdatedelay"],
            )
            return True
        return False
